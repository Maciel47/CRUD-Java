...
public class ExcluirFornecedor extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        int idPessoa = Integer.parseInt(request.getParameter("excluirFornecedor"));
        try{
            GenericDAO dao = new FornecedorDAOImpl(); //Add importação
            dao.excluir(idPessoa);
            request.getRequestDispatcher("ListarFornecedor").forward(request, response);            
        }catch(Exception ex) {
            System.out.println("Erro no Servlet ExcluirFornecedor: " + ex.getMessage());
            ex.printStackTrace();
        }
    }
