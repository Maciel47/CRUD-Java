...
public class ExcluirAdministrador extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        int idPessoa = Integer.parseInt(request.getParameter("excluirAdministrador"));
        try{
            GenericDAO dao = new AdministradorDAOImpl(); //Add importação
            dao.excluir(idPessoa);
            request.getRequestDispatcher("ListarAdministrador").forward(request, response);            
        }catch(Exception ex) {
            System.out.println("Erro no Servlet ExcluirAdministrador: " + ex.getMessage());
            ex.printStackTrace();
        }
    }
